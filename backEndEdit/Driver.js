...
function runGame() {
   ...
   var gameOver = false;
   var activePlayer = data.players[0];
...
function init() {
   ...
   roomLinks(data.rooms);

   var startRooms = {};
   startRooms['Jim'] = data.rooms[7].left;
   startRooms['Pam'] = data.rooms[3].down;
   startRooms['Dwight'] = data.rooms[2].down;
   startRooms['Angela'] = data.rooms[1].right;
   startRooms['Michael'] = data.rooms[8].left;
   startRooms['Andy'] = data.rooms[0].down;

   // Initialize cards
   // six characters
   data.character_cards = new Deck.Deck([
   ...
   // six weapons
   data.weapon_cards = new Deck.Deck([
   ...
   // nine rooms
   var room_cards = [];
   for (let ir = 0; ir < data.rooms.length; ir++) {
      room_cards.push(new Card.Card('room', data.rooms[ir].name, data.rooms[ir].imgURL ))
   }

   data.room_cards = new Deck.Deck(room_cards);


   data.secretFolder = pickCrime(data.character_cards, data.weapon_cards, data.room_cards);

   var allCards = Deck.mergeDecks([data.character_cards, data.weapon_cards, data.room_cards]);
   ...
   for (let ip = users.length - 1; ip >= 0 ; ip--) {
      ...
      if (ip == users.length - 1) {
         ...
         nextPlayer = data.players[ip+1];
      }
      data.players[ip] = new Player.Player(users.ID, users.name, users.charName, startRoom, nextPlayer);
   }

   // Linked list loops around like play around table
   data.players[ip = users.length - 1].next = data.players[0];



   // Deal cards
   let hands = allCards.dealAll(data.players.length);

   // assign hands to players
   for (let ip = 0; ip < data.players.length; ip++) {
      data.players[ip].cardList = hands[ip];
   } 
